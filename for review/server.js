...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.articleId && database.articles[requestComment.articleId]&& requestComment.username && database.users[requestComment.username]&& requestComment.body !== '') {
    const comment = {
...
function deleteComment(url, request) {  
  ...
  if (savedComment) {
    const savedArticle = database.comments[id].articleId;
    const articleCommentIds = database.articles[savedArticle].commentIds;
...
function saveDatabase(){
        try{
    yaml.safeDump('/database.yaml', database);
    }
    catch(e){};
};
